...
for i_big_var=1:length(variation)
    ...
    for i_small_var=1:small_variation
        alpha_small_var = alpha;
        alpha_small_var(alpha_var) = alpha_small_var(alpha_var)*(1+0.5*i_small_var);
        small_plus_F45 = ForwardODE45(alpha_small_var,time_mesh,x_initial);

        alpha_small_var(alpha_var) = alpha_small_var(alpha_var)*(1-0.1*i_small_var);
        small_minus_F45 = ForwardODE45(alpha_small_var,time_mesh,x_initial);
        
        subplot(3,1,1)
        plot(time_mesh,small_minus_F45(1,:),'r',time_mesh,small_plus_F45(1,:),'linewidth',0.5);
        hold on
        subplot(3,1,2)
        plot(time_mesh,small_minus_F45(2,:),'b',time_mesh,small_plus_F45(2,:),'linewidth',0.5);
        hold on
        subplot(3,1,3)
        plot(time_mesh,small_minus_F45(3,:),'m',time_mesh,small_plus_F45(3,:),'linewidth',0.5);
        hold on
    end


    subplot(3,1,1)
    plot(time_mesh,F45(1,:),'r','linewidth',1);
    hold on
    subplot(3,1,2)
    plot(time_mesh,F45(2,:),'b','linewidth',1)
    hold on
    subplot(3,1,3)
    plot(time_mesh,F45(3,:),'m','linewidth',1)
    hold on
end
