  ...
  alpha(alpha_chosen)=variation(i_big_var);
    ...
    for i_small_var=1:small_var
        if i_big_var == 1
            ...
            alpha_plus_var(alpha_chosen,:) = alpha_plus_var(alpha_chosen,:)*(1+0.05*i_small_var);
            small_plus_F45 = ForwardODE45(alpha_plus_var,time_mesh,x_initial);
            time_area = [time_mesh,time_mesh];
            area_big = polyarea(F45_area(1,:), time_area);
            %p_big = polyfit(time_mesh,F45(i_big_var,:),2)
            %func_big = p_big(1)*time_mesh.^2 + p_big(2)*time_mesh + p_big(3);
            %area_big = integral(func_big, time_mesh(1), time_mesh(end))
            %area(end+1) =integral ;
