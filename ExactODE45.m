...
function [g,g_brus,g_add] = ExactODE45(alpha, time_mesh,noise_level,x_initial)%(alpha, time_mesh,obs_start,obs_end,noise_level,x_initial)


    nodes = length(time_mesh); % Number of nodes in the time partition

    g = ForwardODE45(alpha,time_mesh,x_initial);
